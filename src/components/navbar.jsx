import React from "react";
import { OutsideAlerter } from "./wraper";
import { OutsideAlerterc } from "./wraperc";

import { gql } from "@apollo/client";
import { client } from "../App";
import { NavLink } from "react-router-dom";
import classes from "../styles/navbar.module.css";
import { cartContext } from "../Context/Context";

export class Navbar extends React.Component {
  constructor() {
    super();
    this.onClickOutside = this.onClickOutside.bind(this);
    this.onClickOutsidec = this.onClickOutsidec.bind(this);
  }
  state = {
    active: false,
    ON: false,
    Currencies: { currencies: [] },
  };
  onClickOutside() {
    this.setState({ ON: false });
  }
  onClickOutsidec() {
    this.setState({ active: false });
  }

  componentDidMount() {
    const GET_Currencies = gql`
      query {
        currencies {
          label
          symbol
        }
      }
    `;
    client.query({ query: GET_Currencies }).then((res) => {
      const Currencies = res.data;

      this.setState({ Currencies: Currencies });
    });
  }

  render() {
    const {
      addx,

      cart,
      setCurrency,
      choosenCurrency,
      decx,
      choose,
    } = this.context;
    let cartPrices = cart?.map((item) =>
      item?.product.prices?.map((price) => price?.amount)
    );
    // console.log(cart);
    const total = () => {
      if (cartPrices?.length > 0) {
        return choosenCurrency === "USD"
          ? cartPrices?.map((item) => item[0])?.reduce((a, b) => a + b)
          : choosenCurrency === "GBP"
          ? cartPrices?.map((item) => item[1])?.reduce((a, b) => a + b)
          : choosenCurrency === "AUD"
          ? cartPrices?.map((item) => item[2])?.reduce((a, b) => a + b)
          : choosenCurrency === "JPY"
          ? cartPrices?.map((item) => item[3])?.reduce((a, b) => a + b)
          : choosenCurrency === "RUB" &&
            cartPrices?.map((item) => item[4])?.reduce((a, b) => a + b);
      }
    };
    console.log(this.state);
    const toggle = () => {
      if (this.state.active === false) {
        this.setState({ active: true });
      }
    };
    const on = () => {
      if (this.state.ON === false) {
        this.setState({ ON: true });
      }
    };

    return (
      <>
        <div className={classes.navConatiner}>
          <div className={classes.con1}>
            <h3>
              <NavLink
                className={classes.btb}
                to="/"
                exact
                activeClassName={classes.act}
                // className={classes.button}
              >
                Home
              </NavLink>
            </h3>
            <h3>
              <NavLink
                className={classes.btb}
                to="/electronics"
                exact
                activeClassName={classes.act}
              >
                Electronics
              </NavLink>
            </h3>
            <h3>
              <NavLink
                className={classes.btb}
                to="/clothes"
                exact
                activeClassName={classes.act}
              >
                Clothes
              </NavLink>
            </h3>
          </div>
          {/* ////////////////////////////////////////////////////////////////////////// */}
          <svg
            width="41"
            height="41"
            viewBox="0 0 41 41"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <g clip-path="url(#clip0_150_357)">
              <path
                d="M34.0222 28.6646C34.0494 28.983 33.8009 29.2566 33.4846 29.2566H7.46924C7.15373 29.2566 6.90553 28.9843 6.93156 28.6665L8.7959 5.91227C8.8191 5.62962 9.05287 5.41211 9.33372 5.41211H31.5426C31.8226 5.41211 32.0561 5.62853 32.0801 5.91036L34.0222 28.6646Z"
                fill="#1DCF65"
              />
              <path
                d="M36.0988 34.6014C36.1313 34.9985 35.8211 35.339 35.4268 35.339H5.59438C5.2009 35.339 4.89092 35.0002 4.92208 34.6037L7.06376 7.34718C7.09168 6.9927 7.38426 6.71973 7.73606 6.71973H33.1958C33.5468 6.71973 33.8391 6.99161 33.868 7.34499L36.0988 34.6014Z"
                fill="url(#paint0_linear_150_357)"
              />
              <path
                d="M19.9232 26.6953C16.0402 26.6953 12.8813 22.8631 12.8813 18.1528C12.8813 17.9075 13.0782 17.7085 13.3211 17.7085C13.564 17.7085 13.7608 17.9073 13.7608 18.1528C13.7608 22.3732 16.5253 25.8067 19.9234 25.8067C23.3214 25.8067 26.0859 22.3732 26.0859 18.1528C26.0859 17.9075 26.2827 17.7085 26.5257 17.7085C26.7686 17.7085 26.9654 17.9073 26.9654 18.1528C26.9653 22.8631 23.8062 26.6953 19.9232 26.6953Z"
                fill="white"
              />
              <path
                d="M24.2581 18.0337C24.1456 18.0337 24.0331 17.9904 23.9471 17.9036C23.7754 17.7301 23.7754 17.4488 23.9471 17.2753L26.226 14.9729C26.3084 14.8897 26.4203 14.8428 26.5369 14.8428C26.6536 14.8428 26.7654 14.8895 26.8479 14.9729L29.1045 17.2529C29.2762 17.4264 29.2762 17.7077 29.1045 17.8812C28.9327 18.0546 28.6543 18.0547 28.4826 17.8812L26.5368 15.9155L24.569 17.9036C24.4831 17.9904 24.3706 18.0337 24.2581 18.0337Z"
                fill="white"
              />
            </g>
            <defs>
              <linearGradient
                id="paint0_linear_150_357"
                x1="29.8733"
                y1="31.3337"
                x2="11.5132"
                y2="9.9008"
                gradientUnits="userSpaceOnUse"
              >
                <stop stop-color="#52D67A" />
                <stop offset="1" stop-color="#5AEE87" />
              </linearGradient>
              <clipPath id="clip0_150_357">
                <rect
                  width="31.16"
                  height="30.176"
                  fill="white"
                  transform="translate(4.91992 5.41211)"
                />
              </clipPath>
            </defs>
          </svg>

          {/* //////////////////////////////////////////////////////////////////////////// */}
          <div className={classes.con3}>
            {choosenCurrency === "USD" ? (
              <div>{this?.state?.Currencies?.currencies[0]?.symbol}</div>
            ) : choosenCurrency === "GBP" ? (
              <div>{this?.state?.Currencies?.currencies[1]?.symbol}</div>
            ) : choosenCurrency === "AUD" ? (
              <div>{this?.state?.Currencies?.currencies[2]?.symbol}</div>
            ) : choosenCurrency === "JPY" ? (
              <div>{this?.state?.Currencies?.currencies[3]?.symbol}</div>
            ) : (
              choosenCurrency === "RUB" && (
                <div>{this?.state?.Currencies?.currencies[4]?.symbol}</div>
              )
            )}

            <OutsideAlerter func={this.onClickOutside}>
              <div className={classes.currencies}>
                <h4
                  style={{
                    position: "relative",
                    left: "-19px",
                    border: "none",
                    background: "none",
                    fontWeight: "bold",
                    marginRight: "-30px",
                  }}
                  onClick={() => {
                    on();
                  }}
                >
                  <svg
                    width="8"
                    height="4"
                    viewBox="0 0 8 4"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M1 0.5L4 3.5L7 0.5"
                      stroke="black"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    />
                  </svg>
                </h4>
                <div
                  className={
                    this.state.ON === true ? classes.activec : classes.inactive
                  }
                >
                  {this?.state?.Currencies?.currencies?.map((currency) => (
                    <div>
                      <h5
                        onClick={() => {
                          setCurrency(currency.label);
                          this.setState({ ON: false });
                        }}
                      >
                        {" "}
                        {currency.label + currency.symbol}
                      </h5>
                      <hr />
                    </div>
                  ))}
                </div>
              </div>
            </OutsideAlerter>
            {/* //////////////Cart Page Button///////////////////////// */}

            {/* ////////////////////////////CART_LAY_OUT////////////////////////////////////////////// */}
            <div
              className={
                this.state.active === true ? classes.grey : classes.inactive
              }
            >
              .
            </div>
            <div
              className={
                this.state.ON === true ? classes.grey : classes.inactive
              }
            >
              .
            </div>
            <OutsideAlerterc func={this.onClickOutsidec}>
              <svg
                style={{ zIndex: "10" }}
                onClick={() => toggle()}
                width="20"
                height="19"
                viewBox="0 0 20 19"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M19.5613 3.87359C19.1822 3.41031 18.5924 3.12873 17.9821 3.12873H5.15889L4.75914 1.63901C4.52718 0.773016 3.72769 0.168945 2.80069 0.168945H0.653099C0.295301 0.168945 0 0.450523 0 0.793474C0 1.13562 0.294459 1.418 0.653099 1.418H2.80069C3.11654 1.418 3.39045 1.61936 3.47434 1.92139L6.04306 11.7077C6.27502 12.5737 7.07451 13.1778 8.00152 13.1778H16.4028C17.3289 13.1778 18.1507 12.5737 18.3612 11.7077L19.9405 5.50575C20.0877 4.941 19.9619 4.33693 19.5613 3.87365L19.5613 3.87359ZM18.6566 5.22252L17.0773 11.4245C16.9934 11.7265 16.7195 11.9279 16.4036 11.9279H8.00154C7.68569 11.9279 7.41178 11.7265 7.32789 11.4245L5.49611 4.39756H17.983C18.1936 4.39756 18.4042 4.49824 18.5308 4.65948C18.6567 4.81994 18.7192 5.0213 18.6567 5.22266L18.6566 5.22252Z"
                  fill="#43464E"
                />
                <path
                  d="M8.44437 13.9814C7.2443 13.9814 6.25488 14.9276 6.25488 16.0751C6.25488 17.2226 7.24439 18.1688 8.44437 18.1688C9.64445 18.1696 10.6339 17.2234 10.6339 16.0757C10.6339 14.928 9.64436 13.9812 8.44437 13.9812V13.9814ZM8.44437 16.9011C7.9599 16.9011 7.58071 16.5385 7.58071 16.0752C7.58071 15.6119 7.9599 15.2493 8.44437 15.2493C8.92885 15.2493 9.30804 15.6119 9.30804 16.0752C9.30722 16.5188 8.90748 16.9011 8.44437 16.9011Z"
                  fill="#43464E"
                />
                <path
                  d="M15.6875 13.9814C14.4875 13.9814 13.498 14.9277 13.498 16.0752C13.498 17.2226 14.4876 18.1689 15.6875 18.1689C16.8875 18.1689 17.877 17.2226 17.877 16.0752C17.8565 14.9284 16.8875 13.9814 15.6875 13.9814ZM15.6875 16.9011C15.2031 16.9011 14.8239 16.5385 14.8239 16.0752C14.8239 15.612 15.2031 15.2493 15.6875 15.2493C16.172 15.2493 16.5512 15.612 16.5512 16.0752C16.5512 16.5188 16.1506 16.9011 15.6875 16.9011Z"
                  fill="#43464E"
                />
              </svg>
              <span
                style={{
                  position: "relative",
                  left: "1px",
                  top: "4px",
                }}
              >
                {cart?.length}
              </span>
              <div
                className={
                  this.state.active === true ? classes.active : classes.inactive
                }
              >
                <h3>
                  My Bag :{""}
                  {cart?.length} Items
                </h3>
                <hr style={{ width: "100%" }} />
                {cart?.map((product) => (
                  <div>
                    <div className={classes.card}>
                      <div>
                        <NavLink exact to={`/product/${product?.product?.id}`}>
                          <img
                            className={classes.overimg}
                            src={product?.product?.gallery[0]}
                            alt="Girl in a jacket"
                            width="180"
                            height="180"
                          />
                        </NavLink>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          gap: "40px",
                        }}
                      >
                        <button
                          className={classes.btnp}
                          onClick={() => addx(product)}
                        >
                          +
                        </button>
                        <h4>{product?.amount}</h4>
                        <button
                          className={classes.btnm}
                          onClick={() => decx(product)}
                        >
                          -
                        </button>
                      </div>
                      <div>
                        <div
                          style={{
                            padding: "3px ",
                            fontWeight: "bold",
                            fontSize: "15px ",
                          }}
                        >
                          {product?.product?.name}{" "}
                        </div>
                        <div
                          style={{
                            padding: "3px ",
                            fontWeight: "bold",
                            fontSize: "15px ",
                          }}
                        >
                          {" "}
                          {product?.product?.brand}{" "}
                        </div>

                        {choosenCurrency === "USD" ? (
                          <span
                            style={{
                              padding: "3px ",
                              fontWeight: "bold",
                              fontSize: "15px ",
                            }}
                          >
                            {product?.product?.prices[0]?.amount +
                              " " +
                              product?.product?.prices[0]?.currency
                                ?.symbol}{" "}
                          </span>
                        ) : choosenCurrency === "GBP" ? (
                          <span
                            style={{
                              padding: "3px ",
                              fontWeight: "bold",
                              fontSize: "15px ",
                            }}
                          >
                            {product?.product?.prices[1]?.amount +
                              " " +
                              product?.product?.prices[1]?.currency
                                ?.symbol}{" "}
                          </span>
                        ) : choosenCurrency === "AUD" ? (
                          <span
                            style={{
                              padding: "3px ",
                              fontWeight: "bold",
                              fontSize: "15px ",
                            }}
                          >
                            {product?.product?.prices[2]?.amount +
                              " " +
                              product?.product?.prices[2]?.currency
                                ?.symbol}{" "}
                          </span>
                        ) : choosenCurrency === "JPY" ? (
                          <span
                            style={{
                              padding: "3px ",
                              fontWeight: "bold",
                              fontSize: "15px ",
                            }}
                          >
                            {product?.product?.prices[3]?.amount +
                              " " +
                              product?.product?.prices[3]?.currency
                                ?.symbol}{" "}
                          </span>
                        ) : (
                          choosenCurrency === "RUB" && (
                            <span
                              style={{
                                padding: "3px ",
                                fontWeight: "bold",
                                fontSize: "15px ",
                              }}
                            >
                              {product?.product?.prices[4]?.amount +
                                " " +
                                product?.product?.prices[4]?.currency
                                  ?.symbol}{" "}
                            </span>
                          )
                        )}

                        <div>
                          {product?.product?.attributes.map((attr) => {
                            return (
                              <div>
                                <div
                                  style={{
                                    padding: "3px ",
                                    fontWeight: "bold",
                                    fontSize: "15px ",
                                  }}
                                >
                                  {attr.name} :
                                </div>
                                {attr.items.map((item) => (
                                  <div>
                                    <button
                                      onClick={() =>
                                        choose(product, item?.value, attr?.name)
                                      }
                                      style={{
                                        backgroundColor: item?.value
                                          ?.split("")
                                          ?.includes("#")
                                          ? item?.value
                                          : "white",
                                        color: item?.value
                                          ?.split("")
                                          ?.includes("#")
                                          ? item?.value
                                          : "black",
                                        border: item?.value
                                          .split("")
                                          ?.includes("#")
                                          ? "none"
                                          : "1px solid black",
                                        width: "40px",
                                        fontSize: "10px",
                                        cursor: "pointer",
                                      }}
                                    >
                                      {item?.value?.split("")?.includes("#")
                                        ? item?.displayValue
                                        : item?.value}
                                    </button>
                                  </div>
                                ))}
                              </div>
                            );
                          })}
                        </div>
                        <div>
                          <h3>You Selected :</h3>
                          {Object.values(product?.attr).map((val) => (
                            <h4
                              style={{
                                backgroundColor: val.split("")?.includes("#")
                                  ? val
                                  : "white",
                                color: val?.split("")?.includes("#")
                                  ? val
                                  : "black",

                                border: val.split("")?.includes("#")
                                  ? `1px solid ${val}`
                                  : "1px solid black",

                                width: "80px ",
                                textAlign: "center",
                                padding: "5px",

                                margin: "2px ",
                              }}
                            >
                              {" "}
                              {val}{" "}
                            </h4>
                          ))}
                        </div>
                      </div>
                    </div>
                    <hr />
                  </div>
                ))}{" "}
                <div>
                  <div
                    style={{ display: "flex", justifyContent: "space-between" }}
                  >
                    <h4> Total : </h4>
                    {choosenCurrency === "USD" ? (
                      <h4>
                        {total()}
                        {this?.state?.Currencies?.currencies[0]?.symbol}
                      </h4>
                    ) : choosenCurrency === "GBP" ? (
                      <h4>
                        {total()}{" "}
                        {this?.state?.Currencies?.currencies[1]?.symbol}
                      </h4>
                    ) : choosenCurrency === "AUD" ? (
                      <h4>
                        {total()}
                        {this?.state?.Currencies?.currencies[2]?.symbol}
                      </h4>
                    ) : choosenCurrency === "JPY" ? (
                      <h4>
                        {total()}
                        {this?.state?.Currencies?.currencies[3]?.symbol}
                      </h4>
                    ) : (
                      choosenCurrency === "RUB" && (
                        <h4>
                          {total()}{" "}
                          {this?.state?.Currencies?.currencies[4]?.symbol}
                        </h4>
                      )
                    )}
                  </div>
                  <NavLink className={classes.btm4} to="/cart">
                    <button className={classes.btm2}> View Bag </button>
                  </NavLink>
                  <NavLink className={classes.btm3} to="/order">
                    <button className={classes.btm}>Order</button>
                  </NavLink>
                </div>
              </div>
              {/* ///////////////////////////////////////////////////END OF CART LAY OUT////////////////////////////////// */}
            </OutsideAlerterc>
          </div>
        </div>
      </>
    );
  }
}
Navbar.contextType = cartContext;
